...
outputFile = '%s/output/analyzeHitFit_from%s%s%s_%s_%s_%s_elecChannel.root'%( os.getenv( "CMSSW_BASE" ), era, etaBins, ptBins, sample, jecLevel, matchAlgorithm )
...
process.hltHighLevel.HLTPaths          = [ 'pf2PatPathElectrons'
                                         ]
process.hltHighLevelReference = process.hltHighLevel.clone( HLTPaths = [ 'referencePathElectrons'
                                                                       ]
                                                          )
