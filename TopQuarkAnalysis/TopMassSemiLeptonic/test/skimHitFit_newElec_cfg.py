...
electronUsePV        = False
#electronSelectBase   = 'ecalDrivenMomentum.pt > 20. && (chargedHadronIso+max(0.,neutralHadronIso+photonIso-1.*userIsolation("User1Iso")))/pt < 0.15 && abs(eta) < 3.0 && (1. >= electronID("mvaTrigV0") && electronID("mvaTrigV0") >= 0.)'
electronSelectBase   = 'pt > 20. && (chargedHadronIso+max(0.,neutralHadronIso+photonIso-1.*userIsolation("User1Iso")))/pt < 0.15 && abs(eta) < 3.0 && (1. >= electronID("mvaTrigV0") && electronID("mvaTrigV0") >= 0.)'
electronSelectHitFit = electronSelectBase + ' && abs(eta) < 2.5 && passConversionVeto'
electronSelect       = electronSelectBase + ' && abs(eta) < 2.5'
#electronSelectSignal = 'ecalDrivenMomentum.pt > 30. && abs(eta) < 2.5 && !(1.4442 < abs(superCluster.eta) && abs(superCluster.eta) < 1.5660) && abs(dB) < 0.02 && electronID("mvaTrigV0") > 0.5 && (chargedHadronIso+max(0.,neutralHadronIso+photonIso-1.0*userIsolation("User1Iso")))/pt < 0.1 && passConversionVeto && gsfTrack.trackerExpectedHitsInner.numberOfHits <= 0'
electronSelectSignal = 'pt > 30. && abs(eta) < 2.5 && !(1.4442 < abs(superCluster.eta) && abs(superCluster.eta) < 1.5660) && abs(dB) < 0.02 && electronID("mvaTrigV0") > 0.5 && (chargedHadronIso+max(0.,neutralHadronIso+photonIso-1.0*userIsolation("User1Iso")))/pt < 0.1 && passConversionVeto && gsfTrack.trackerExpectedHitsInner.numberOfHits <= 0'
...
, fileName       = cms.untracked.string( 'skimHitFit_newElec.root' )
...
outputFile = '%s/output/skimHitFit_%s_newElec.root'%( os.getenv( "CMSSW_BASE" ), mc )
if runTest:
...
process.selectedPatElectronsMCMatch.src = cms.InputTag( 'calibratedPatElectrons' )
process.selectedPatElectrons.src        = cms.InputTag( 'calibratedPatElectrons' )
process.selectedPatElectronsHitFit.src  = cms.InputTag( 'calibratedPatElectrons' )
